...
|---|---|---|
| **Database** | `supabase/` | The real Supabase/PostgreSQL backend: 26 migrations, full RLS, triggers, RPC functions, reporting views, seed data and a 198-assertion verification suite. |
...
> **Local demo mode vs. Supabase mode.** When `VITE_SUPABASE_URL` and
> `VITE_SUPABASE_ANON_KEY` are absent the app runs in *local demo mode*. When
> they are present the same pages call the live project through
> `@supabase/supabase-js`. To switch, copy `.env.example` to `.env.local` and
> fill in the two values (never the `service_role` key).
...
│   │   ├── supabase.ts         client + demo/live mode switch
│   │   ├── database.types.ts   generated-style row types for the schema
