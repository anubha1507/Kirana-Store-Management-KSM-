...
export function BillingPage() {
  ...
  const products = useMemo(() => {
    ...
    return engine.state.products
      .filter((p) => !p.deleted_at && (!q || p.name.toLowerCase().includes(q)))
      .slice(0, 40);
  }, [engine, query]);

  const detailed = (engine ? cart : []).map((l) => {
    const p = engine!.state.products.find((x) => x.id === l.product_id);
  ...
  if (!engine) return <p className="muted">Supabase mode: billing loads from the backend.</p>;
  return (
    <div className="page">
      <div className="page-head"><div><h2>Billing / POS</h2>
      <p className="muted">Role: {role} · prices from catalogue, totals at checkout.</p></div>
      <span className="pill">Next: {engine.peekInvoice()}</span></div>
        ...
        <div className="card"><div className="card-head"><h3>Bill ({cart.length})</h3></div>
          ...
          <div className="form-grid">
            ...
            <label>Customer<select value={customerId} onChange={(e) => setCustomerId(e.target.value)}>
              ...
              {engine.state.customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select></label>
